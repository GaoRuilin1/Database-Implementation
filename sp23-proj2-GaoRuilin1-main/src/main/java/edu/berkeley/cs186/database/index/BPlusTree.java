...
public class BPlusTree {
    ...
    public Optional<RecordId> get(DataBox key) {
        ...
        if (inner != null) return inner.getKey(key);
    ...
    public Iterator<RecordId> scanGreaterEqual(DataBox key) {
        ...
        if (root != null) {
            LeafNode leftN = root.get(key);
            return new BPlusTreeIterator(leftN, leftN.scanGreaterEqual(key));
        }
        else{
    ...
    public void put(DataBox key, RecordId rid) {
        ...
        if (insertPair.isPresent()) {
            ...
            List<DataBox> keys = new ArrayList<>();
            //add the old root's key
            keys.add(insertPair.get().getFirst());
            //add the old root and the new root
    ...
    public void bulkLoad(Iterator<Pair<DataBox, RecordId>> data, float fillFactor) {
        ...
        if (left != root || left.scanAll().hasNext()) {
            throw new BPlusTreeException("The tree is not empty");
        }
        while (data.hasNext()) {
            //insert pair
            Optional<Pair<DataBox, Long>> insertPair = this.root.bulkLoad(data, fillFactor);
            //if the root splits
            if (insertPair.isPresent()) {
                List<Long> children = new ArrayList<>();
                List<DataBox> keys = new ArrayList<>();
    ...
    private class BPlusTreeIterator implements Iterator<RecordId> {
        ...
        LeafNode nextNode;
        public BPlusTreeIterator(LeafNode leftNode, Iterator<RecordId> leftNodeIterator) {
            this.nextNode = leftNode;
            this.nextIterator = leftNodeIterator;
        }
        @Override
        public boolean hasNext() {
            // TODO(proj2): implement
            if (nextIterator.hasNext() || nextNode.getRightSibling().isPresent()) {
                return true;
            }return false;
            }
