...
class LeafNode extends BPlusNode {
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        //if keys already contains the key, throw an exception
        if (keys.contains(key)) {
            throw new BPlusTreeException("key already exists");
        }
        int d = metadata.getOrder();
        //put the key and the rid into the leaf
        ...
        if (keys.size() <= 2 * d) {
            ...
            LeafNode rightLeaf = new LeafNode(metadata, bufferManager, rightKeys, rightRids,
                    rightSibling, treeContext);
            //update the rightSibling
            //update the keys and rids
            this.keys = leftKeys;
            this.rids = leftRids;
            rightSibling = Optional.of(rightLeaf.getPage().getPageNum());
            //sync the leaf
            sync();
            //return the key and the right leaf
            return Optional.of(new Pair<>(rightKeys.get(0), rightLeaf.getPage().getPageNum()));
        }
        }
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        // TODO(proj2): implement
        int d = metadata.getOrder();
        //if the leaf is empty, throw an exception
        if (fillFactor * 2 * d <= 0) {
            throw new BPlusTreeException("bulk-load to empty leaves not provided.");
        }
    ...
    public void remove(DataBox key) {
        ...
        // if the key is not in the leaf, do nothing
        //else remove the key and the rid
        if (index != -1) {
            keys.remove(index);
            rids.remove(index);
        }
    ...
    public static LeafNode fromBytes(BPlusTreeMetadata metadata, BufferManager bufferManager,
        ...
        Optional<Long> rightSiblings;
        // if rightSibling is -1, then there is no right sibling
        if (rightSibling == -1 ) rightSiblings = Optional.empty();
        else rightSiblings = Optional.of(rightSibling);
        ...
        return  new LeafNode(metadata, bufferManager, page, keys,
                recordIds, rightSiblings, treeContext);
