...
class InnerNode extends BPlusNode {
    ...
    public LeafNode get(DataBox key) {
        ...
        BPlusNode targetNode = getChild(targetChildrenIndex);
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        ...
        Optional<Pair<DataBox, Long>> one = child.put(key, rid);

        // if there is no need to split, then return. lzq I love you forever
        if (!one.isPresent()) return Optional.empty();
        // logic for split
        Pair<DataBox, Long> p = one.get();
        // serialize the new child
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        int d = metadata.getOrder();
        // if the node is not full, then add the data
        while (data.hasNext() && keys.size() <= 2 * d) {
            BPlusNode rightChild = getChild(children.size() - 1);
            Optional<Pair<DataBox, Long>> load = rightChild.bulkLoad(data, fillFactor);
            if (load.isPresent()) {
                keys.add(keys.size(), load.get().getFirst());
                children.add(children.size(), load.get().getSecond());
            }
        }
